...
<div class="mb-2">
	...
	{#if $pancreas.width}
		<span
			>Kasos latako plotis {$pancreas.width}cm<NormShow
				cat={TXT.DUCTUS_CAT}
				dog={TXT.DUCTUS_DOG}
			/>.</span
		>
	...
	{#if $lymphnodes.numberPancrDuo === 1 && $lymphnodes.sizePancrDuo}
		<span
			>Stebimas 1 {$lymphnodes.sizePancrDuo}s kasos ir dvylikapirštės žarnos limfinis mazgas.</span
		>
	...
	{:else if $lymphnodes.numberPancrDuo}
		<span
			>Stebimi {$lymphnodes.numberPancrDuo} kasos ir dvylikapirštės žarnos limfiniai mazgai.</span
		>
	{/if}

	{#if $lymphnodes.echogenPancrDuo && $lymphnodes.numberPancrDuo > 1}
		<span
			>Kasos ir dvylikapirštės žarnos lm. parenchima yra {$lymphnodes.echogenPancrDuo} juos supantiems
			riebalams.</span
		>
	{:else if $lymphnodes.echogenPancrDuo}
		<span
			>Kasos ir dvylikapirštės žarnos limfinio mazgo parenchima yra {$lymphnodes.echogenPancrDuo} ją
			supantiems riebalams.</span
		>
	{/if}
