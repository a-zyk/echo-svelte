<script>
	...
	const addLympnode = () => {
		$lymphnodesStore.lumbarRen = [...$lymphnodesStore.lumbarRen, {}];
	};

	const removeLympnode = () => {
		$lymphnodesStore.lumbarRen = $lymphnodesStore.lumbarRen.slice(0, -1);
	};
</script>

<Toggle classes="card">
	<div class="mb-2 flex gap-2 text-base" slot="title">
		Juosmeninės aortos/inkstiniai
		<Tooltip
			tooltipText="Lokalizacija: išilgai aortos, šalia inksto. Drenuoja: inkstus, prieinksčius, šlapimo pūslę, lyties organus, pilvaplėvę."
		/>
	</div>

	<div slot="content">
		<div class="flex flex-col md:flex-row md:gap-9">
			<NumberInput bind:value={$lymphnodesStore.numberLumbarRenal} title="Limfinių m. skaičius" />
			<div>
				<div class="titles">Juosmeninės aortos/inkstiniai lm. yra</div>
				<div class="radio">
					<RadioBtn bind:group={$lymphnodesStore.sizeLumbarRenal} value="padidėję" name="sizeLumbarRenal" />
					<RadioBtn
						bind:group={$lymphnodesStore.sizeLumbarRenal}
						value="nepadidėję"
						name="sizeLumbarRenal"
					/>
				</div>
			</div>
		</div>
		...
		<div class="flex gap-2">
			<div class="titles">Juosmeninės aortos/inkstinis lm. parenchima ją supantiems riebalams yra</div>
			<Tooltip
		...
		<div class="radio">
			<RadioBtn
				bind:group={$lymphnodesStore.echogenLumbarRenal}
			...
			{#if $lymphnodesStore.lumbarRen.length > 1}
				<button
		...
		<Textarea
			...
			bind:value={$lymphnodesStore.otherChangesLumbarRenal}
