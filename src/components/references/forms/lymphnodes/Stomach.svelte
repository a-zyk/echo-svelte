<script>
	...
	const addLympnode = () => {
		$lymphnodesStore.stomach = [...$lymphnodesStore.stomach, {}];
	};

	const removeLympnode = () => {
		$lymphnodesStore.stomach = $lymphnodesStore.stomach.slice(0, -1);
	};
</script>
...
<Toggle classes="card">
	<div class="mb-2 flex gap-2 text-base" slot="title">
		Skrandžio
		<Tooltip
			tooltipText="Lokalizacija: šalia prievarčio (pylorus), arčiau mažosios skrandžio kreivės. Drenuoja: dvylikapirštę, kasą ir taukinę."
		/>
	</div>

	<div slot="content">
		<div class="flex flex-col md:flex-row md:gap-9">
			<NumberInput bind:value={$lymphnodesStore.numberStomach} title="Limfinių m. skaičius" />
			<div>
				<div class="titles">Skrandžio lm. yra</div>
				<div class="radio">
					<RadioBtn bind:group={$lymphnodesStore.sizeStomach} value="padidėję" name="sizeStomach" />
					<RadioBtn
						bind:group={$lymphnodesStore.sizeStomach}
						value="nepadidėję"
						name="sizeStomach"
					/>
				</div>
		...
		<div class="flex gap-2">
			<div class="titles">Skrandžio lm. parenchima ją supantiems riebalams yra</div>
			<Tooltip
		...
		<div class="radio">
			<RadioBtn
				bind:group={$lymphnodesStore.echogenStomach}
		...
		{#if $lymphnodesStore.stomach.length > 1}
			<button
		...
		<Textarea
			...
			bind:value={$lymphnodesStore.otherChangesStomach}
