export const NODULAR_HYPER_TEXT = `Senatvinis pakitimas dažnesnis katėms. Stebimi daugybiniai hipoechoiški mazgeliai 0.3 – 1cm diametro. Gali atrodyti kaip cistos. Gali atrodyt panašiai į insulinoma. Dažniausiai katės neturi jokių klinikinių simptomų. Gali būti susiję su persirgtu pankreatitu.`
export const NODULAR_HYPER_DESCR = `Šuo, mazgelinė hiperplazija dėl lėtinio pankreatito.`

export const DUCTUS_TEXT = `Senyvo amžiaus katėms kasos latako skersmuo yra norma iki 0.25cm. Dažniausiai nebūna jokių klinikinių požymių. Kasos latakas gali plėsti ir dėl letinio ir ūmaus pankratito.`
export const DUCTUS_DESCR = `Sena katė, kasos latako dilatacija.`

export const PANCREATITIS_TEXT = `Dėl ūmaus pankreatito kasa padidėja, tampa hipoechoiška ją supančiam riebaliniui audiniui.Šalia esanti dvylikapirštė žarna gali būti koruguota, atoniška. Kasos kraštai suapvalėja arba kapsulė tampa nelygi. Vystosi riebalinio audinio steatitas, jis tampa hiperechoiškas gali būti laisvų skysčių.`
export const PANCREATITIS_DESCR = `Šuo, pankreatitas.P- kasa, matomas hiperechoiškas riebalinis audinys, sustorėjusi ir edemiška dvylikapirštė.`

export const ABSCESS_TEXT = `Kasos cistos ir abscesai yra nedažna pankreatito komplikacija. Manoma, kad dėl uždegimo pratrūksta kasos latakas, kasos ar aplinkiniuose audiniuose susiformuoja fibrotinė kapsulė pripildyta kasos išskiriamo skysčio ir fermentų. Diferencijacijai atliekama citologija, histopatologija.`
export const ABSCESS_DESCR = `Katė, kasos abscesas.`

export const NEOPLASIA_TEXT = `Kasos endokrininio audinio neoplazijos (glukagonomos, insulinomos ir gastrinomos) yra nedažnas reiškinys. Šunims dažniausiai pasitaiko insulinomos. Matomos kaip masės kasos skiltyse. Kasos adenocarcinoma retas vėžys, šunims gali būti nustatomas kaip masė ar difuziški pakitimai, katėms dažniausiai difuziškai apimantis visą kasą.`
export const NEOPLASIA_DESCR = `Šuo, kasos karcinoma.`